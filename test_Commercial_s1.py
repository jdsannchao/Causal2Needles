...
def main(args):
    ...
    counter_correct = 0  # number of questions with correct answer
    for movie_id in tqdm(movies):
        ...
        for question, nested_list in questions.items():
            ...
            gt_answer = left_expansion+1
            logger.info(
                f"Cause: ({cause_idx}) {full_texts[cause_idx]}Effect: ({effect_idx}) {full_texts[effect_idx]}"
                f"Cause_padding: ({left_expansion})  Effect_padding: ({right_end-effect_idx})")
            ...
            if 'gemini' in args.model_id:
                answer = generate_answer_gemini(images, context, s1_question, prompt, args)

            logger.info(
                f"Question: {s1_question}\nGT: {gt_answer}\nAnswer: {answer}")
            counter_all += 1

            if answer:
                answer = answer.rstrip("\n")

                if contains_single_digit(answer, str(gt_answer)):
                    counter_correct += 1
    ...
    logger.info(f"num_all: {counter_all}   num_correct: {counter_correct}   precision: {counter_correct/counter_all}")
