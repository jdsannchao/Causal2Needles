...
def main(args):
    ...
    if 'gemini' in args.model_id:
        ...
        processor = AutoProcessor.from_pretrained("../HuggingFace/Qwen/Qwen2.5-VL-7B-Instruct", trust_remote_code=True)
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained("../HuggingFace/Qwen/Qwen2.5-VL-7B-Instruct", torch_dtype="auto", device_map="auto")

    elif args.model_id == 'qwen2.5vl-32b-instruct':
        processor = AutoProcessor.from_pretrained("../HuggingFace/Qwen/Qwen2.5-VL-32B-Instruct", trust_remote_code=True)
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained("../HuggingFace/Qwen/Qwen2.5-VL-32B-Instruct", torch_dtype="auto", device_map="auto")
        model.eval()
    ...
    for dataset_name in ['yms', 'symon']:
        ...
        for movie_id in tqdm(movies):
            ...
            for question, info in questions.items():
                cause_idx, effect_idx = info[0]

                context = ' '.join(full_texts)
                left_pad = min(random.randint(0, args.expansion), cause_idx)
                right_end = min(effect_idx + args.expansion - left_pad, max_idx)

                if pre_causal_pair != info[0]:
                    gt_answer = left_pad + 1
                else:
                    gt_answer = left_pad + effect_idx - cause_idx + 1
                pre_causal_pair = info[0]

                if effect_idx - cause_idx > 21:
                    logger.info(f"Skip too long causal pair: {cause_idx}-{effect_idx}")
                    continue
